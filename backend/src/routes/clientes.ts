...
const linhaClienteSchema = z.object({
  ...
  Email: z.string().email('email invalido').optional().or(z.literal('')).transform((v) => v || null),
...
router.post('/', async (req, res) => {
  ...
  if (!nome) return res.status(400).json({ error: { message: 'Nome e obrigatorio.' } });
  const { rows } = await pool.query(
    `INSERT INTO clientes (nome, cpf_cnpj, email, whatsapp, endereco, cidade, observacoes) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *`,
...
router.put('/:id', async (req, res) => {
  const { nome, cpfCnpj, email, whatsapp, endereco, cidade, observacoes } = req.body;
  const { rows } = await pool.query(
    `UPDATE clientes SET nome=$1, cpf_cnpj=$2, email=$3, whatsapp=$4, endereco=$5, cidade=$6, observacoes=$7, atualizado_em=now() WHERE id=$8 RETURNING *`,
    [nome, cpfCnpj ?? null, email ?? null, whatsapp ?? null, endereco ?? null, cidade ?? null, observacoes ?? null, req.params.id],
    );
  if (!rows[0]) return res.status(404).json({ error: { message: 'Cliente nao encontrado.' } });
  res.json(rows[0]);
});
...
router.post('/importar', upload.single('arquivo'), async (req, res) => {
  ...
  try {
    ...
    for (const linha of validos) {
      await client.query(
        `INSERT INTO clientes (nome, cpf_cnpj, email, whatsapp, endereco, cidade, observacoes) VALUES ($1,$2,$3,$4,$5,$6,$7)`,
        [linha.Nome, linha.CPF_CNPJ, linha.Email, linha.WhatsApp, linha.Endereco, linha.Cidade, linha.Observacoes],
        );
      importados++;
    }
    await client.query(
      `INSERT INTO importacoes (entidade, nome_arquivo, total_linhas, total_importados, total_erros, detalhes_erros) VALUES ('clientes', $1, $2, $3, $4, $5)`,
