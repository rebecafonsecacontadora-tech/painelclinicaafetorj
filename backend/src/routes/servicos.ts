...
const linhaServicoSchema = z.object({
  ...
  Valor: z.union([z.string(), z.number()]).transform((v) => Number(String(v).replace(',', '.'))).refine((v) => !isNaN(v) && v >= 0, 'valor invalido'),
...
router.post('/', async (req, res) => {
  ...
  if (!nome || valor === undefined) {
    return res.status(400).json({ error: { message: 'Nome e valor sao obrigatorios.' } });
  }
...
router.put('/:codigo', async (req, res) => {
  const { nome, valor, descricao } = req.body;
  const { rows } = await pool.query(
    `UPDATE servicos SET nome=$1, valor=$2, descricao=$3, atualizado_em=now() WHERE codigo_tabelado=$4 RETURNING *`,
    [nome, valor, descricao ?? null, req.params.codigo],
    );
  if (!rows[0]) return res.status(404).json({ error: { message: 'Servico nao encontrado.' } });
  res.json(rows[0]);
});
...
router.post('/importar', upload.single('arquivo'), async (req, res) => {
  ...
  try {
    ...
    await client.query(
      `INSERT INTO importacoes (entidade, nome_arquivo, total_linhas, total_importados, total_erros, detalhes_erros) VALUES ('servicos', $1, $2, $3, $4, $5)`,
