...
interface Props {
  /** endpoint relativo, ex: '/clientes/importar' */
  endpoint: string;
  /** chamado após importação bem-sucedida, para recarregar a listagem */
  onConcluido: () => void;
  /** URL de um modelo de planilha para o usuário baixar (opcional) */
  urlModelo?: string;
}
  ...
  return (
    <div className="importar-planilha">
      <input
        ...
        style={{ display: 'none' }}
      />
      <label htmlFor="input-importar-planilha" className="botao-secundario">
        {carregando ? 'Importando...' : 'Importar planilha'}
      </label>
      {urlModelo && (
        <a href={urlModelo} download className="link-modelo">
          Baixar modelo
        </a>
      )}
